...
import { debug, defaultSettings, links, text } from '@/misc/config';
...
const IndexPopup = () => {
  ...
  const [showSettings, setShowSettings] = useStorage('showSettings', (x) =>
    x === undefined ? defaultSettings : x,
  );
    ...
    <div className="App text-center w-[240px] font['Roboto', sans-serif] text-xl font-light bg-slate-900 text-white p-5 select-none gap-2 flex flex-col">
      ...
      <div className="flex justify-center align-middle">
        <div className="grid grid-cols-2 gap-4 m-auto">
          <div className="flex align-middle justify-center">
            <AiOutlineSetting
              className={`m-auto w-16 h-16 bg-slate-600 p-2 cursor-pointer ${
                menuTab === menuOptions.settings
                  ? 'bg-slate-500'
                  : 'bg-slate-600'
              } hover:outline-offset-2 outline-white outline`}
          ...
          <div className="flex align-middle justify-center">
            <AiOutlineInfoCircle
              className={`m-auto w-16 h-16 bg-slate-600 p-2 cursor-pointer ${
                menuTab === menuOptions.info ? 'bg-slate-500' : 'bg-slate-600'
              } hover:outline-offset-2 outline-white outline`}
          ...
          <div className="flex align-middle justify-center">
            <DiGoogleAnalytics
              ...
              onClick={() =>
                openTab(browser.runtime.getURL('tabs/consent.html'))
              }
            />
          </div>
          <div className="flex align-middle justify-center">
      ...
      {menuTab === menuOptions.settings && (
        <div className="flex justify-center align-middle m-2">
          <div
            className="grid justify-center align-middle grid-cols-1 gap-2 m-auto w-44"
            ...
            <div className="flex justify-center m-auto bg-slate-800 align-middle w-full hover:outline-offset-2 outline-white outline">
              <div className="m-auto p-1">{text.enableOverlay}</div>
              ...
              <Switch
                ...
                onChange={() =>
                  setShowSettings({
                    ...showSettings,
                    overlayButton: !showSettings.overlayButton,
                  })
                }
            ...
            <div className="flex justify-center m-auto bg-slate-800 align-middle w-full hover:outline-offset-2 outline-white outline">
              <div className="m-auto p-1">{text.enableSearchButton}</div>
              ...
              <Switch
                ...
                onChange={() =>
                  setShowSettings({
                    ...showSettings,
                    searchButton: !showSettings.searchButton,
                  })
                }
                inputProps={{ 'aria-label': 'controlled' }}
            ...
            <div className="flex justify-center m-auto bg-slate-800 align-middle w-full hover:outline-offset-2 outline-white outline">
              <div className="m-auto p-1">{text.funMode}</div>

              <Switch
                checked={funMode}
                onChange={() => setFunMode(!funMode)}
                inputProps={{ 'aria-label': 'controlled' }}
                aria-label={text.funMode}
                className="m-auto"
              />
            </div>
          </div>
      ...
      {menuTab === menuOptions.info && (
        <div className="text-base m-2 p-2 bg-slate-800">{text.info.text}</div>
      )}
    </div>
  );
};
